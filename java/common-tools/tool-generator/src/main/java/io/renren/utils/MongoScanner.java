...
public class MongoScanner {
    ...
    public List<String> groupAggregation(Integer skip, Integer limit) throws MongoCommandException {
        ...
        BasicDBObject $project =
                new BasicDBObject(
                        "$project",
                        new BasicDBObject(
                                "arrayofkeyvalue", new BasicDBObject("$objectToArray", "$$ROOT")));
    ...
    public Set<String> getNextParameterNames(String parameterName) {
        ...
        Document project =
                new Document(
                        "$project", new Document("list", "$" + parameterName).append("_id", false));
        ...
        if (isArray.hasNext()) {
            ...
            for (int i : TYPE) {
                ...
                if (iterator.hasNext()) {
                    ...
                    result.setType(i);
                    logger.info(
                            "解析[" + propertyName + "]是[" + Type.typeInfo(result.getType()) + "]");
                    return result;
                }
            }
            result.setType(2);
